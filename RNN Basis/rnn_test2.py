...
test_columns_to_keep = ['Retweet Count', 'Mention Count', 'Follower Count', 'Bot Label', 'Year', 'Month', 'Day'] + \
...
def create_rnn_model_with_attention(units1=128, units2=64, dropout_rate=0.5, learning_rate=0.001):
    model = Sequential()
...
rnn_attention_train_probs = best_rnn_attention.predict(X_train_rnn).flatten()
rnn_attention_test_probs = best_rnn_attention.predict(X_test_rnn).flatten()
...
# Add username to LIME explainer
...
if len(bot_indices) > 0:
    ...
    exp = explainer.explain_instance(X_test.iloc[i].values, lambda x: np.hstack((1 - best_rnn_attention.model_.predict(np.reshape(x, (x.shape[0], X_test_rnn.shape[1], 1))), best_rnn_attention.model_.predict(np.reshape(x, (x.shape[0], X_test_rnn.shape[1], 1))))), num_features=10)
